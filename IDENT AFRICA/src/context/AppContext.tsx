...
const EXCHANGE_RATES: Record<Currency, { rate: number; symbol: string; prefix: boolean }> = {
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [destinations, setDestinations] = useState<Destination[]>(() => {
    try {
      const saved = localStorage.getItem('safariflow_destinations');
  ...
  const [hotels, setHotels] = useState<LuxuryLodge[]>(() => {
    try {
      const saved = localStorage.getItem('safariflow_hotels');
      return saved ? JSON.parse(saved) : MOCK_LODGES;
    } catch {
      return MOCK_LODGES;
    }
  });

  const [itineraries, setItineraries] = useState<SafariItinerary[]>(() => {
    try {
      const saved = localStorage.getItem('safariflow_itineraries');
      return saved ? JSON.parse(saved) : MOCK_ITINERARIES;
    } catch {
