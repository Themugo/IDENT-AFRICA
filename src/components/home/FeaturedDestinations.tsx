  ...
  return (
    ...
    >
      ...
      <div className="relative h-64 overflow-hidden bg-[#2D2621]">
        ...
        <div className="absolute top-4 left-4 px-3 py-1 bg-[#2D2621]/90 text-[#D6B06A] text-[10px] font-bold uppercase tracking-widest border border-[#C89A4B]/80 rounded-md shadow-md z-10 font-mono">
          {dest.country}
        </div>
        ...
        <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between text-white text-xs z-10">
          <span className="px-2.5 py-1 bg-[#FFF8EC]/90 text-[#2A1E17] border border-[#C89A4B]/50 text-[10px] font-bold uppercase tracking-wider rounded-md backdrop-blur-sm shadow">
            {dest.category}
          </span>
          <div className="flex items-center gap-1 font-bold bg-[#C89A4B] text-[#1A1008] px-2.5 py-1 text-[11px] rounded-md shadow-md">
            ...
            <span className="text-[9px] opacity-90">({dest.reviewsCount})</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
        <div>
          ...
          <p className="mt-3 text-xs text-[#5A4738] line-clamp-2 leading-relaxed font-normal">
            {dest.description}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 pt-2">
          {dest.wildlifeHighlights.slice(0, 2).map((highlight) => (
            <span
              key={highlight}
              className="px-2.5 py-1 text-[9px] font-bold uppercase tracking-widest bg-[#F5E7D0] text-[#2A1E17] border border-[#C89A4B]/40 rounded-md font-mono"
            >
              {highlight}
            </span>
          ))}
        ...
        <div className="pt-4 border-t border-[#C89A4B]/30 flex items-center justify-between">
          <div>
            <span className="text-[9px] uppercase font-bold tracking-widest text-[#5A4738] block font-mono">
              From / Person
            </span>
          ...
          <button
            ...
            className="btn-gold text-[10px] py-2.5 px-4 rounded-lg flex items-center gap-1.5 font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm"
          >
...
export const FeaturedDestinations: React.FC = () => {
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-[#463D34] text-[#F4E8D5] border-b border-[#C89A4B]/40 relative overflow-hidden texture-savannah">
      <div className="max-w-7xl mx-auto">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 pb-6 border-b border-[#C89A4B]/30">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#D6B06A] mb-3 block font-mono">
