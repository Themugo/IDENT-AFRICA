...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full transition-all duration-300 bg-[#2D2621]/95 backdrop-blur-md border-b border-[#C89A4B]/50 text-[#F4E8D5] shadow-2xl texture-leather">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-3 lg:gap-4">
        ...
        >
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-[#FFF8EC] border-2 border-[#4F6848] flex items-center justify-center p-0.5 shadow-md group-hover:scale-105 transition-all shrink-0">
            <AcaciaTreeIcon className="w-6 h-6 sm:w-7 sm:h-7" leafColor="#4F6848" trunkColor="#2D3E2B" />
          </div>
          <div className="leading-tight">
            <span className="text-sm sm:text-base lg:text-lg font-cinzel font-black tracking-wider text-[#F4E8D5] group-hover:text-[#D6B06A] transition-colors block">
              IDENT AFRICA
            </span>
            <span className="block text-[7.5px] sm:text-[8.5px] tracking-[0.2em] text-[#D6B06A] uppercase font-bold whitespace-nowrap">
              Africa's Travel Ecosystem
            </span>
          </div>
        </div>

        {/* Desktop Navigation - Clean Proportional Links */}
        <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1.5 text-[10px] xl:text-[11px] font-bold uppercase tracking-wider text-[#D3C5AE]">
          {primaryNavItems.map((item) => {
            ...
            if (item.isAI) {
              return (
                <button
                  ...
                  className="transition-all flex items-center gap-1 h-8 px-2.5 xl:px-3 rounded-full bg-[#4F6848] text-[#FFF8EC] border border-[#C89A4B]/60 hover:bg-[#2D3E2B] shadow-md font-bold cursor-pointer whitespace-nowrap ml-0.5 group"
                >
                  <Sparkles className="w-3 h-3 text-[#D6B06A] group-hover:rotate-12 transition-transform shrink-0" />
            ...
            return (
              <button
                key={item.page}
                onClick={() => navigateTo(item.page)}
                className={`transition-all flex items-center gap-1 h-8 px-2 xl:px-2.5 rounded-lg whitespace-nowrap cursor-pointer border ${
                  isActive
              ...
              >
                ...
                {item.page === 'compare' && comparedItineraryIds.length > 0 && (
                  <span className="ml-0.5 px-1.5 py-0.2 text-[8.5px] font-mono font-bold bg-[#2D2621] text-[#D6B06A] rounded-full border border-[#C89A4B]">
          ...
          <div className="relative ml-0.5">
            <button
              ...
              className="h-8 px-1.5 rounded-lg border border-[#C89A4B]/40 hover:border-[#D6B06A] hover:bg-[#463D34] text-[#D3C5AE] hover:text-[#D6B06A] transition-all flex items-center gap-1 cursor-pointer"
              title="More Ecosystem Portals"
            >
              <MoreHorizontal className="w-3.5 h-3.5 text-[#D6B06A]" />
              <ChevronDown className="w-2.5 h-2.5 text-[#D6B06A]" />
            </button>

            {overflowMenuOpen && (
              <div
                onMouseLeave={() => setOverflowMenuOpen(false)}
                className="absolute right-0 mt-2 w-64 bg-[#2D2621] border-2 border-[#C89A4B]/70 shadow-2xl rounded-2xl p-2 z-50 text-left texture-leather space-y-1"
              >
                <div className="px-3 py-1.5 border-b border-[#C89A4B]/30 mb-1 text-[9px] font-mono font-bold uppercase tracking-widest text-[#D6B06A]">
                ...
                {overflowNavItems.map((sub) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-[#D3C5AE] block font-normal leading-tight">
                        {sub.description}
                      </span>
                    </div>
                  </button>
        ...
        <div className="hidden lg:flex items-center gap-1.5 xl:gap-2 shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="h-8 px-2.5 border border-[#C89A4B]/60 text-[10px] font-bold uppercase tracking-wider text-[#F4E8D5] hover:bg-[#C89A4B] hover:text-[#2D2621] transition-all flex items-center gap-1.5 bg-[#463D34] rounded-lg cursor-pointer"
              title="Global Currency Switcher"
            >
              <span className="font-mono font-black text-xs text-[#D6B06A]">{CURRENCY_INFO[currency].symbol}</span>
              <span>{currency}</span>
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" title="Live Exchange Rates Active" />
              <ChevronDown className="w-3 h-3 text-[#D6B06A]" />
            </button>

            {currencyDropdownOpen && (
              <div className="absolute right-0 mt-2 w-72 bg-[#2D2621] border-2 border-[#C89A4B] shadow-2xl rounded-2xl p-3 z-50 overflow-hidden font-mono text-xs text-[#F4E8D5]">
                
                <div className="flex items-center justify-between border-b border-[#C89A4B]/30 pb-2 mb-2">
                  <div className="flex items-center gap-1.5 text-[#D6B06A] text-[10px] font-bold uppercase">
                  ...
                  <button
                    ...
                    className="p-1 hover:bg-[#C89A4B]/20 text-[#D6B06A] rounded transition-all flex items-center gap-1 text-[9px] cursor-pointer"
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] opacity-80 block">{info.name}</span>
                          </div>
                        </div>

                        <span className="text-[10px] font-bold bg-[#2D2621] text-[#D6B06A] px-2 py-0.5 rounded-md border border-[#C89A4B]/30">
                          {rateDisplay}
                        </span>
                      </button>
                    );
                ...
                <div className="mt-2 pt-2 border-t border-[#C89A4B]/20 flex items-center justify-between text-[9px] text-[#D3C5AE]">
                  <span>Source: {ratesSource === 'live' ? 'Open FX Engine' : 'Standard Baseline'}</span>
                  <span className="text-emerald-400 font-bold">● Auto Convert</span>
                </div>
          ...
          <button
            onClick={toggleTheme}
            className="w-8 h-8 border border-[#C89A4B]/50 text-[#D6B06A] hover:bg-[#C89A4B]/20 transition-colors bg-[#463D34] rounded-lg cursor-pointer flex items-center justify-center"
          ...
          <button
            ...
            className="relative w-8 h-8 border border-[#C89A4B]/50 text-[#F4E8D5] hover:bg-[#C89A4B]/20 transition-colors bg-[#463D34] rounded-lg cursor-pointer flex items-center justify-center"
            title="Saved Expeditions & Portfolios"
          >
            <Bookmark className="w-3.5 h-3.5 text-[#D6B06A]" />
            {totalSaved > 0 && (
              <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#C89A4B] text-[#2D2621] text-[8px] font-mono font-black flex items-center justify-center rounded-full border border-[#2D2621]">
                {totalSaved}
              </span>
            )}
          </button>
          ...
          <button
            ...
            className="btn-gold h-8 px-3 rounded-lg text-[10px] sm:text-[11px] font-bold uppercase tracking-wider whitespace-nowrap cursor-pointer shadow-lg flex items-center justify-center"
          >
            Book Expedition
          </button>
        </div>
        ...
        <div className="lg:hidden flex items-center gap-2">
          ...
          >
            ...
            {totalSaved > 0 && (
              <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#C89A4B] text-[#2D2621] text-[8px] font-mono font-black flex items-center justify-center rounded-full">
                {totalSaved}
              </span>
            )}
          </button>
