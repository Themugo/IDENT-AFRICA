...
import { auditDatabaseHealth } from '../db/validation.js';
...
/**
 * GET /api/admin/db-health - Database Integrity & Health Audit
 */
router.get('/db-health', async (req: Request, res: Response) => {
  try {
    const healthData = await auditDatabaseHealth();
    res.status(200).json(createResponse(true, healthData));
  } catch (error) {
    console.error('Error conducting database health audit:', error);
    res.status(500).json(createResponse(false, undefined, 'Failed to perform database audit', String(error)));
  }
});
